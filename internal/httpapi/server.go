...
type storeAPI interface {
	...
	GetBoard(ctx context.Context, pc *store.ProjectContext, tagFilter string, searchFilter string, assigneeFilter string, sprintFilter store.SprintFilter) (store.Project, []store.TagCount, []store.WorkflowColumn, map[string][]store.Todo, error)
	GetBoardPaged(ctx context.Context, pc *store.ProjectContext, tagFilter string, searchFilter string, assigneeFilter string, sprintFilter store.SprintFilter, limitPerLane int) (store.Project, []store.TagCount, []store.WorkflowColumn, map[string][]store.Todo, map[string]store.LaneMeta, error)
	ListTagCounts(ctx context.Context, pc *store.ProjectContext) ([]store.TagCount, error)
	ListTodosForBoardLane(ctx context.Context, projectID int64, columnKey string, limit int, afterRank, afterID int64, tagFilter, searchFilter string, assigneeFilter string, sprintFilter store.SprintFilter) ([]store.Todo, string, bool, error)
