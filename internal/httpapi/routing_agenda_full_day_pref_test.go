...
func TestAgendaFullDayPreference_DoesNotModifyProjectAgendaSettings(t *testing.T) {
	...
	ownerJSON := bootstrapUserClient(t, ownerClient, ts.URL, "Owner", "agenda-full-day-owner@example.com", "password123")
	...
	if strings.Contains(string(body), "agendaTimeline") || strings.Contains(string(body), `"timeline"`) {
		t.Fatalf("board payload included timeline preference: %s", body)
	}
...
func TestAgendaFullDayPreference_IsPerUser(t *testing.T) {
	ts, sqlDB, cleanup := newTestHTTPServer(t, "full")
	defer cleanup()

	ownerClient := newCookieClient(t)
	bootstrapUserClient(t, ownerClient, ts.URL, "Owner", "agenda-full-day-a@example.com", "password123")
	...
	other, err := st.CreateUser(context.Background(), "agenda-full-day-b@example.com", "password123", "Other")
	if err != nil {
	...
	resp, body = doJSON(t, ownerClient, http.MethodGet, ts.URL+"/api/user/preferences?key=agendaFullDay", nil, &ownerPref)
	if resp.StatusCode != http.StatusOK {
		t.Fatalf("GET owner preference: status=%d body=%s", resp.StatusCode, string(body))
	}
	if ownerPref["value"] != "true" {
		t.Fatalf("owner preference=%v, want true", ownerPref["value"])
	}

	var otherPref map[string]any
	resp, body = doJSON(t, otherClient, http.MethodGet, ts.URL+"/api/user/preferences?key=agendaFullDay", nil, &otherPref)
	if resp.StatusCode != http.StatusOK {
